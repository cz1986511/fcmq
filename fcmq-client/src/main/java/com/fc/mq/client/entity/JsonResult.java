package com.fc.mq.client.entity;
 ...
 * 4. {@link JsonResult#getStatus()}不为0表示服务方法执行失败，通过{@link #getMsg()}
 * 获取消息描述信息（错误信息）。<br />
 * <p>
 * <strong>消息代码{@link #getStatus()}、消息描述{@link #getMsg()}使用规范</strong><br />
 * 1. 绝大部分服务方法不需要使用消息代码。<br />
 * &nbsp;&nbsp;&nbsp;如<strong>前端ajax接口基本规范</strong>中所示，前端通过
 * <code>getSatus()</code>确定服务方法是否执行成功， 如果执行失败，通过<code>getMsg()</code>得到错误描述即可；<br />
 * &nbsp;&nbsp;&nbsp;服务器端发生异常执行失败时，详细的错误描述、堆栈信息应当记录到数据库或者服务端的文件日志中，用于问题排查处理；
 * 然后通过<code>setError(String errCode, String msg)</code>
 * 返回对用户友好的、业务型的描述信息，前端开发者通过这个信息可以大致了解、定位问题所在，详细的排查处理则通过服务端日志完成。
